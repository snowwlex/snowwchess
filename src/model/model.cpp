...
void Model::notifyUpdate() const {
    for (SUBSCRIBERS::const_iterator itSubscriber = mySubscribers.begin(); itSubscriber != mySubscribers.end(); ++itSubscriber) {
        (*itSubscriber)->updateIt();
        qDebug() << ":Model:" << "notifyUpdate() : calling updateIt()";
    }
}
...
void Model::makeMoveEffectCastle(const Move& move) {
    qDebug() << ":Model:" << " castling searching the castle rule";
    CastleRule castleRule = myRules->getCastleRule(move.pos2.myX, move.pos2.myY, move.player);
    qDebug() << ":Model:" << " castling " << castleRule.dx << castleRule.dy << " "
            << castleRule.kingCell.myX << castleRule.kingCell.myY << " "
            << castleRule.player << " "
            << castleRule.rookCellStart.myX << castleRule.rookCellStart.myY << " "
            << castleRule.rookCellEnd.myX << castleRule.rookCellEnd.myY;
