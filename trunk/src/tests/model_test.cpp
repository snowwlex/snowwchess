...
void ModelTest::run() {
    ...
    checksWithEmptyRules();
    wrongRules();

}

void ModelTest::checksWithEmptyRules() {
    ...
    TEST(model->allMoves(WHITE).empty() == true);
    TEST(model->allMoves(BLACK).empty() == true);
    TEST(model->movesFromPosition(Position(4, 6)).empty() == true);
    TEST(model->getBoardSizeX() == 0);
    TEST(model->getBoardSizeY() == 0);

    SimpleMove sMove;
    sMove.pos1 = Position(0, 0);
    sMove.pos2 = Position(2, 2);
    TEST(model->canMove(sMove) == false);
    TEST(model->makeMove(sMove) == false);

    CHECK_THROWS(model->checkGameStatus(WHITE));
    CHECK_THROWS(model->isCheck(BLACK));
    CHECK_THROWS(model->getPlayerData(WHITE));
    TEST(model->getBoardCell(10, 10).setId == -1);
    TEST(model->getCurrentPlayer() == 0);
    TEST(model->getFigureIdOnBoard(10, 10) == -1);
